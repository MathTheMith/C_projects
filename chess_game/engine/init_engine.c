...
{
    int piece    = game->board[game->old_x][game->old_y];
    int captured = game->board[x][y];

    uint64_t old_bit = 1ULL << (game->old_y * 8 + game->old_x);
    uint64_t new_bit = 1ULL << (y * 8 + x);

    *get_bitboard(game, piece) ^= (old_bit | new_bit);

    if (captured != EMPTY)
        *get_bitboard(game, captured) ^= new_bit;
}

void undo_move(t_game *game, int piece, int from_x, int from_y, int to_x, int to_y, int captured)
{
    uint64_t old_bit = 1ULL << (from_y * 8 + from_x);
    uint64_t new_bit = 1ULL << (to_y * 8 + to_x);
