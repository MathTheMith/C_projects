...
void show_possibles_moves(int board[BOARD_SIZE][BOARD_SIZE], int x, int y) {
    if (board[x][y] == WR || board[x][y] == BR) show_rook_moves(x, y, board);
    if (board[x][y] == WB || board[x][y] == BB) show_bishop_moves(x, y, board);
    if (board[x][y] == WN || board[x][y] == BN) show_knight_moves(x, y, board);
    if (board[x][y] == WQ || board[x][y] == BQ) show_queen_moves(x, y, board);
    if (board[x][y] == WK || board[x][y] == BK) show_king_moves(x, y, board);
    if (board[x][y] == WP) show_wpawns_moves(x, y, board);
    if (board[x][y] == BP) show_bpawns_moves(x, y, board);
}
